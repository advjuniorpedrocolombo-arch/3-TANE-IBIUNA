/* PROVA ADMINISTRAÇÃO PÚBLICA — BACKEND V2 (JSONP/GET)
 * Motivo: evitar bloqueios CORS entre GitHub Pages e Google Apps Script.
 * IMPORTANTE: substituir o código do Apps Script por este arquivo e publicar NOVA VERSÃO da implantação existente.
 */
...
function doGet(e){
  try{
    ...
    else if(action==='salvarResposta') out=salvarRespostasLote_({email:p.email,respostas:parseJson_(p.respostas,[])});
    else if(action==='salvarRespostas') out=salvarRespostasLote_({email:p.email,respostas:parseJson_(p.respostas,[])});
...
function responder_(obj,callback){
  ...
  if(callback){
    return ContentService.createTextOutput(String(callback)+'('+txt+');').setMimeType(ContentService.MimeType.JAVASCRIPT);
  }
...
function iniciarOuRetomar_(p){
  ...
  try{
    ...
    const sel=shuffle_(qs).slice(0,total),ord=sel.map(q=>q.id),opt={};sel.forEach(q=>opt[q.id]=shuffle_(q.alternativas.map(a=>a.id)));
...
function consultarTentativa_(email){const e=normEmail_(email);if(!emailOk_(e))return{ok:false,erro:'E-mail inválido.'};const t=localizar_(e);return t?montarRetorno_(t.row):{ok:false,existe:false,erro:'Tentativa não encontrada.'}}
...
function salvarRespostasLote_(p){
  ...
  try{
    ...
    const sh=respostasSheet_();const status=String(sh.getRange(t.row,COL.STATUS).getDisplayValue()||'');if(statusFinal_(status))return{ok:false,finalizada:true,erro:'Prova já finalizada.'};
    const tempo=calcularTempo_(t.row);if(tempo.restanteSegundos<=0){finalizarLinha_(t.row,'FINALIZADA POR TEMPO');return montarRetorno_(t.row)}
    const estado=lerEstado_(t.row),validas=validarRespostas_(estado,respostas);validas.forEach(r=>estado.answers[r.questionId]=r.optionId);estado.lastSavedAt=new Date().toISOString();
...
function finalizarPorAluno_(p){
  ...
  try{
    const t=localizar_(email);if(!t)return{ok:false,erro:'Tentativa não encontrada.'};
    const estado=lerEstado_(t.row),respostas=Array.isArray(p.respostas)?p.respostas:[];validarRespostas_(estado,respostas).forEach(r=>estado.answers[r.questionId]=r.optionId);respostasSheet_().getRange(t.row,COL.ESTADO_JSON).setValue(JSON.stringify(estado));
    const st=String(respostasSheet_().getRange(t.row,COL.STATUS).getDisplayValue()||'');if(!statusFinal_(st))finalizarLinha_(t.row,calcularTempo_(t.row).restanteSegundos<=0?'FINALIZADA POR TEMPO':'FINALIZADA PELO ALUNO');SpreadsheetApp.flush();return montarRetorno_(t.row);
  }finally{lock.releaseLock()}
}
...
function montarRetorno_(row){
  ...
  if(out.finalizada){out.acertos=vals[10];out.nota=vals[11];out.mencao=vals[19]||vals[12];out.envio=iso_(vals[6])}return out;
}

function finalizarLinha_(row,status){const sh=respostasSheet_(),corr=corrigir_(row),t=calcularTempo_(row),cfg=lerConfig_();sh.getRange(row,COL.ENVIO).setValue(new Date());sh.getRange(row,COL.TEMPO_UTILIZADO).setValue(segRelogio_(Math.min(t.decorridoSegundos,t.totalSegundos)));sh.getRange(row,COL.TEMPO_REFERENCIA).setValue(Number(cfg.TEMPO_MINUTOS||30));sh.getRange(row,COL.ACERTOS).setValue(corr.acertos);sh.getRange(row,COL.NOTA).setValue(corr.nota);sh.getRange(row,COL.MENCAO_AUTO).setValue(corr.mencao);sh.getRange(row,COL.MENCAO_FINAL).setValue(corr.mencao);sh.getRange(row,COL.STATUS).setValue(status)}
...
function lerConfig_(){const sh=configSheet_(),v=sh.getRange(2,1,Math.max(0,sh.getLastRow()-1),2).getValues(),o={};v.forEach(r=>{if(r[0])o[String(r[0]).trim()]=r[1]});return o}
...
function ss_(){return SpreadsheetApp.openById(APP.SPREADSHEET_ID)}function questoesSheet_(){return ss_().getSheetByName(APP.SHEET_QUESTOES)}function respostasSheet_(){return ss_().getSheetByName(APP.SHEET_RESPOSTAS)}function configSheet_(){return ss_().getSheetByName(APP.SHEET_CONFIG)}
function normEmail_(v){return String(v||'').trim().toLowerCase()}function emailOk_(v){return /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(v)}function criarId_(){return 'ADM-'+Utilities.formatDate(new Date(),APP.TIMEZONE,'yyyyMMdd-HHmmss')+'-'+Math.floor(1000+Math.random()*9000)}function iso_(v){if(!v)return null;const d=new Date(v);return isNaN(d)?null:d.toISOString()}function segRelogio_(s){s=Math.max(0,Math.floor(s));return [Math.floor(s/3600),Math.floor((s%3600)/60),s%60].map(n=>String(n).padStart(2,'0')).join(':')}

function onOpen(){SpreadsheetApp.getUi().createMenu('PROVA ADM. PÚBLICA').addItem('Testar backend V2','testarBackendV2').addToUi()}
function testarBackendV2(){const sh=respostasSheet_();SpreadsheetApp.getUi().alert('Backend V2 OK\nPlanilha: '+ss_().getName()+'\nAba respostas: '+sh.getName()+'\nQuestões: '+lerQuestoes_().length)}
